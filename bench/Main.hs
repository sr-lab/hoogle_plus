...
execExercise :: (String, String) -> IO (String, Maybe Double) -- total time of first solution
execExercise (name, ty) = do
  -- run exercise
  let log = logsDir ++ "/" ++ name ++ ".log"
  readCreateProcessWithExitCode (shell $ command ty log) ""
  hPutStrLn stderr $ command ty log
  hFlush stderr
  -- read results
  file <- readFile log
  let ls = lines file
